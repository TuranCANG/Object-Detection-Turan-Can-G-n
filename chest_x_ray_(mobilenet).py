# -*- coding: utf-8 -*-
"""Chest X-Ray (MobileNet)

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/11jfRw2vxFmL225Zcphl5uIYTP-y2PXZ6
"""
...
import tensorflow as tf # Machine learning library to construct CNN
...
files.upload()  # Uploading kaggle.json to download the dataset
...
# ImageNet pre-trained model which aims to lower power consumption as well as high accuracy towards to image classificaiton.
# Freezing layers is useful to keep pre-trained layers to adapt new training
...
# Run the model once with fake data to be ensure shape inference
dummy_input = np.random.random((1, IMG_HEIGHT, IMG_WIDTH, 3))
model(dummy_input)

# Checking model summary one more time
model.summary()

# Defining parameter of callback to achieve best accuracy while prevent overfitting situation
...
# Convert predicted probabilities to class labels in terms of binary classificaiton (0 or 1)
